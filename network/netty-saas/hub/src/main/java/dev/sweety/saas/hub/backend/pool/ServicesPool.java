...
public class ServicesPool extends DynamicBackendNodePool<ServiceNode> {
    ...
    private final Map<ServiceType, CounterBalancer> clusterPolicies = new ConcurrentHashMap<>();
    ...
    public void setPolicy(ServiceType type, CounterBalancer balancer) {
        this.clusterPolicies.put(type, balancer);
        ServiceCluster existing = clusters.get(type);
        if (existing != null) {
            clusters.put(type, new ServiceCluster(type, balancer));
        }
    }

    public void setPolicy(ServiceType type, Balancers balancer) {
        setPolicy(type, balancer.get());
    }

    private ServiceCluster getCluster(ServiceType type) {
        return clusters.computeIfAbsent(type, t -> new ServiceCluster(t, clusterPolicies.getOrDefault(t, Balancers.ROUND_ROBIN.get())));
    }
    ...
    @Override
    public ServiceNode next(Packet packet, ChannelHandlerContext ctx) {
        if (packet instanceof InternalPacket internal && internal.hasRequest()) {
            final int receiverId = internal.getRequest().receiverId();
            if (receiverId >= 0) {
                final ServiceType type = ServiceType.of(receiverId);
                final ServiceCluster cluster = clusters.get(type);
                if (cluster != null && !cluster.isEmpty()) {
                    ServiceNode selected = cluster.nextNode(packet, ctx);
                    if (selected != null) return selected;
                }
            }
        }
        return super.next(packet, ctx);
    }
