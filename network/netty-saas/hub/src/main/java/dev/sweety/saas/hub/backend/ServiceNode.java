...
public class ServiceNode extends BackendNode implements IService {
    ...
    public void onPacketReceive(final ChannelHandlerContext ctx, final Packet packet) {
        ...
        if (packet instanceof SystemPong) {
            final Duration pingDuration = lastPing != null ? Duration.between(lastPing, Instant.now()) : Duration.ZERO;
        ...
        } else if (packet instanceof SystemConnectionTransaction systemConnection) {
        ...
        } else if (packet instanceof MonitoringMetricReportTransaction monitoring) {
            if (monitoring.hasRequest()) {
