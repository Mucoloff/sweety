...
public abstract class Backend extends Client implements IBackend {
    ...
    public Backend(final String hubHost, final int hubPort, final PacketRegistry packetRegistry, final int localPort) {
        ...
        this.backendLogger.profile("init").info("Waiting for loadbalancer...");
        this.constructor = (i, ts, data) -> {
            try {
                return packetRegistry.constructPacket(i, ts, ((PacketBuffer) data).getBytes());
            } catch (Exception e) {
                this.backendLogger.profile("codec").warn("Failed to construct packet id " + i, e);
                return null;
            }
        };
    ...
    private void sendMetrics() {
        ...
        try {
            ...
            ch.writeAndFlush(metricsPacket);
        } catch (Throwable throwable) {
            backendLogger.profile("metrics").error(throwable);
        }
    }
    ...
    public void onPacketReceive(final ChannelHandlerContext ctx, final Packet packet) {
        ...
        if (internal.hasResponse()) {
            if (!transactions.completeResponse(internal, ctx)) {
                backendLogger.profile("expired").warn(internal.requestCode());
            }
            return;
        }
    ...
    public void exception(final ChannelHandlerContext ctx, final Throwable throwable) {
        if (!reconnect.onException(throwable)) backendLogger.profile("exception").error(throwable);
    }

    @Override
    public void join(final ChannelHandlerContext ctx, final ChannelPromise promise) {
        backendLogger.profile("connect").info("backend " + localPort + " connected");
        promise.setSuccess();
    }
    ...
    public final void quit(final ChannelHandlerContext ctx, final ChannelPromise promise) {
        sampler.reset();
        backendLogger.profile("disconnect").info("backend " + localPort + " disconnected");
        promise.setSuccess();
        reconnect.onQuit();
        this.leave(ctx);
    }
