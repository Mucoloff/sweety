...
/**
 * Fastutil-style primitive enum map: {@code char} values in an ordinal-indexed array, no boxing on
 ...
 */
public final class Enum2CharMap<E extends Enum<E>> implements Map<E, Character> {

    /** The JDK has no {@code ObjCharConsumer} — this fills the gap for {@link #forEachChar}. */
    @FunctionalInterface
    public interface ObjCharConsumer<E> {
        void accept(E key, char value);
    }
    ...
    private Enum2CharMap(Class<E> keyType) {
        this.keyType = keyType;
        this.universe = keyType.getEnumConstants();
        this.values = new char[universe.length];
        this.present = new long[(universe.length + 63) >> 6];
    }

    public static <E extends Enum<E>> Enum2CharMap<E> of(Class<E> keyType) {
    ...
    public char put(E key, char value) {
        int ordinal = key.ordinal();
        char previous = values[ordinal];
    ...
    public char get(E key) {
        return values[key.ordinal()];
    }

    public char getOrDefault(E key, char defaultValue) {
        int ordinal = key.ordinal();
    ...
    public void forEachChar(ObjCharConsumer<E> action) {
        for (E key : universe) {
    ...
    public boolean containsValue(Object value) {
        if (!(value instanceof Character target)) return false;
        for (E key : universe) {
    ...
    @Override
    public Character put(E key, Character value) {
        Objects.requireNonNull(value, "value");
        int ordinal = key.ordinal();
        Character previous = isPresent(ordinal) ? values[ordinal] : null;
        if (!isPresent(ordinal)) {
            markPresent(ordinal);
            size++;
        }
    ...
    @Override
    public void putAll(Map<? extends E, ? extends Character> m) {
        for (Map.Entry<? extends E, ? extends Character> e : m.entrySet()) put(e.getKey(), e.getValue());
    }

    private EnumMap<E, Character> snapshot() {
        EnumMap<E, Character> out = new EnumMap<>(keyType);
        for (E key : universe) {
    ...
    @Override
    public Set<Entry<E, Character>> entrySet() {
