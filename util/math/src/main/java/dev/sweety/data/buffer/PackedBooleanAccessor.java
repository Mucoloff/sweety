...
public interface PackedBooleanAccessor<Self extends BufferReader & BufferWriter> {
    ...
    default PackedBooleanAccessor<Self> writeBoolean(boolean value) {
        //noinspection unchecked
        Self self = (Self) this;
        byte writeMaskIndex = writeMaskIndex();
        byte writeMask = writeMask();

        if ((writeMaskIndex & 7) == 0) {
            writePosIndex(self.writerIndex());
            writeMask = 0;
            self.writeByte((byte) 0);
        }

        if (value) writeMask |= (byte) (1 << (writeMaskIndex & 7));
    ...
    default boolean readBoolean() {
        ...
        boolean result = ((readMask >> (readMaskIndex & 7)) & 1) != 0;
