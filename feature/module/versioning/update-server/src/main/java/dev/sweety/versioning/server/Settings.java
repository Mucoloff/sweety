 ...
 *   <tr><td>{@code UPDATE_SERVER_ROOT}</td><td>{@link dev.sweety.versioning.server.store.Storage}</td><td>Server filesystem root; default {@code storage}</td></tr>
 *   <tr><td>{@code NETTY_HANDSHAKE_SECRET}</td><td>this class, if absent from JSON</td><td>Must match launcher {@code LUCE_HANDSHAKE_SECRET} for Netty HMAC</td></tr>
 ...
 *   <tr><td>{@code LUCE_HANDSHAKE_SECRET}</td><td>launcher client</td><td>Same value as {@code NETTY_HANDSHAKE_SECRET} on server when HMAC is enabled</td></tr>
 * </table>
 *
...
public class Settings {
     ...
     * requires a matching proof from the client. The launcher uses the same value via env {@code LUCE_HANDSHAKE_SECRET}.
     */
    public static String NETTY_HANDSHAKE_SECRET = "";

    /**
     * When non-blank, {@code GET /release/base-jar} and {@code POST /release/download-token} require header {@code X-Luce-Release-Key}.
    ...
    /** Inbound webhook upload throttle (bytes/sec). */
    public static float UPLOAD_SPEED = 5 * 1024 * 1024;
    /** Hard cap on webhook request body size (bytes). */
    public static int MAX_WEBHOOK_BODY = 50 * 1024 * 1024;
    ...
    /** Shared secret for HMAC-SHA256 artifact integrity tag. Blank disables the HMAC header. */
    public static String INTEGRITY_HMAC_SECRET = "";
    /**
     * Path to an Ed25519 private key PEM (PKCS#8) used to sign artifact digests. Blank disables the
     * Ed25519 header. Clients verify with the matching public key. Resolved from env
     * {@code LUCE_ED25519_KEY} when absent here.
     */
    public static String ED25519_KEY_PATH = "";
